...
// model
import AuctionContent, { AuctinoContentIndex } from 'auction/model/Auction';
...
class AuctionServerAgent extends ReverseInquiryServerAgent {
  ...
  public async fetchAuctionContent(contentIndex: number) {
    const result = await this._contractInstance.getContent(contentIndex, { from: this.getUserAccount() });
    const content: AuctionContent = {
      id: result[AuctinoContentIndex.id],
  ...
  public async fetchAuctionContents() {
    const auctionContentLength = await this._contractInstance.getContentsLength();
